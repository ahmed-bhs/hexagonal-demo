...
/**
 * Validateur Domain pour AttribuerCadeauCommand.
 *
 * Validation pure (PHP) sans dépendance framework.
 * Respecte l'architecture hexagonale : logique métier dans le Domain.
 *
 * @implements ValidatorInterface<AttribuerCadeauCommand>
 */
final readonly class AttribuerCadeauCommandValidator implements ValidatorInterface
{
    private const UUID_PATTERN = '/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i';

    public function validate(object $command): array
    {
        assert($command instanceof AttribuerCadeauCommand);
